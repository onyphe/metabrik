...
DEATH
      ...
      die <<'DEATH';
FATAL: The CPAN::local::lib --self-contained flag has never worked reliably and the
original author, Mark Stosberg, was unable or unwilling to maintain it. As
such, this flag has been removed from the CPAN::local::lib codebase in order to
prevent misunderstandings and potentially broken builds. The CPAN::local::lib authors
...
sub deactivate {
  ...
  if (!grep { $_ eq $path } @active_lls) {
    warn "Tried to deactivate inactive CPAN::local::lib '$path'\n";
    return $self;
  }
...
CPAN::local::lib - create and use a local lib/ for perl modules with PERL5LIB

=head1 SYNOPSIS

In code -

  use CPAN::local::lib; # sets up a local lib at ~/perl5

  use CPAN::local::lib '~/foo'; # same, but ~/foo

  # Or...
  use FindBin;
  use CPAN::local::lib "$FindBin::Bin/../support";  # app-local support library

From the shell -

  # Install LWP and its missing dependencies to the '~/perl5' directory
  perl -MCPAN -MCPAN::local::lib -e 'CPAN::install(LWP)'

  # Just print out useful shell commands
  $ perl -MCPAN::local::lib
...
From a .bashrc file -
  ...
  [ $SHLVL -eq 1 ] && eval "$(perl -I$HOME/perl5/lib/perl5 -MCPAN::local::lib)"

=head2 The bootstrapping technique

A typical way to install CPAN::local::lib is using what is known as the
"bootstrapping" technique.  You would do this if your system administrator
hasn't already installed CPAN::local::lib.  In this case, you'll need to install
CPAN::local::lib in your home directory.

Even if you do have administrative privileges, you will still want to set up your
environment variables, as discussed in step 4. Without this, you would still
install the modules into the system CPAN installation and also your Perl scripts
will not use the lib/ path you bootstrapped with CPAN::local::lib.

By default CPAN::local::lib installs itself and the CPAN modules into ~/perl5.
...
Download and unpack the CPAN::local::lib tarball from CPAN (search for "Download"
on the CPAN page about CPAN::local::lib).  Do this as an ordinary user, not as root
...
In order to install CPAN::local::lib into a directory other than the default, you need
...
Run this: (CPAN::local::lib assumes you have make installed on your system)
...
way:
  ...
  echo '[ $SHLVL -eq 1 ] && eval "$(perl -I$HOME/perl5/lib/perl5 -MCPAN::local::lib)"' >>~/.bashrc
...
If you are using C shell, you can do this as follows:
  ...
  echo $SHELL
  /bin/csh
  echo 'eval `perl -I$HOME/perl5/lib/perl5 -MCPAN::local::lib`' >> ~/.cshrc

If you passed to bootstrap a directory other than default, you also need to
give that as import parameter to the call of the CPAN::local::lib module like this
way:

  echo '[ $SHLVL -eq 1 ] && eval "$(perl -I$HOME/foo/lib/perl5 -MCPAN::local::lib=$HOME/foo)"' >>~/.bashrc
...
applications independently, you can use one bootstrapped CPAN::local::lib
installation to install modules in different directories directly this way:

  cd ~/mydir1
  perl -MCPAN::local::lib=./
  eval $(perl -MCPAN::local::lib=./)  ### To set the environment for this shell alone
...
If you are working with several C<CPAN::local::lib> environments, you may want to
remove some of them from the current environment without disturbing the others.
You can deactivate one environment like this (using bourne sh):

  eval $(perl -MCPAN::local::lib=--deactivate,~/path)
...
You can also ask C<CPAN::local::lib> to clean itself completely out of the current
...
In C<< ~/mydir1/scripts/myscript.pl >>:
  ...
  use strict;
  use warnings;
  use CPAN::local::lib "$FindBin::Bin/..";  ### points to ~/mydir1 and local::lib finds lib
...
C<CMD.exe>, you can use this:
  ...
  C:\>perl -MCPAN::local::lib > %TEMP%\tmp.bat && %TEMP%\tmp.bat && del %TEMP%\tmp.bat
  ### instead of $(perl -MCPAN::local::lib=./)
...
CPAN::local::lib also supports PowerShell, and can be used with the
C<Invoke-Expression> cmdlet.

  Invoke-Expression "$(perl -MCPAN::local::lib)"
...
CPAN::local::lib solves this problem by allowing you to create your own directory of
...
CPAN::local::lib arranges things so that your locally installed version of the Perl
...
More generally, CPAN::local::lib allows for the bootstrapping and usage of a
directory containing Perl modules outside of Perl's C<@INC>. This makes it
easier to ship an application with an app-specific copy of a Perl module, or
collection of modules. Useful in cases like when an upstream maintainer hasn't
applied a patch to a module of theirs that you need for your application.

On import, CPAN::local::lib sets the following environment variables to appropriate
values:
...
Options are values that can be passed to the C<CPAN::local::lib> import besides the
directory to use. They are specified as C<use CPAN::local::lib '--option'[, path];>
or C<perl -MCPAN::local::lib=--option[,path]>.

=head2 --deactivate

Remove the chosen path (or the default path) from the module search paths if it
was added by C<CPAN::local::lib>, instead of adding it.

=head2 --deactivate-all

Remove all directories that were added to search paths by C<CPAN::local::lib> from the
search paths.
...
Prevents C<CPAN::local::lib> from creating directories when activating dirs.  This is
...
Returns a list of active C<CPAN::local::lib> paths, according to the
...
Constructs a new C<CPAN::local::lib> object, representing the current state of
...
An arrayref representing active C<CPAN::local::lib> directories.
...
If set, C<CPAN::local::lib> will not try to create directories when activating them.
...
Constructs a new C<CPAN::local::lib> object based on the existing one, overriding the
...
Constructs a new instance with all C<CPAN::local::lib> directories deactivated.
...
Returns a string to set up the C<CPAN::local::lib>, meant to be run by a shell.
...
Be careful about using CPAN::local::lib in combination with "make install UNINST=1".
The idea of this feature is that will uninstall an old version of a module
before installing a new one. However it lacks a safety check that the old
version and the new version will go in the same directory. Used in combination
with CPAN::local::lib, you can potentially delete a globally accessible version of a
module while installing the new version in a local place. Only combine "make
install UNINST=1" and CPAN::local::lib if you understand these possible consequences.
...
workaround you can do is moving your CPAN::local::lib to a directory with spaces
B<after> you installed all modules inside your CPAN::local::lib bootstrap. But be
...
=item * On VMS and MacOS Classic (pre-OS X), CPAN::local::lib loads L<File::Spec>.
This means any L<File::Spec> version installed in the CPAN::local::lib will be
ignored by scripts using CPAN::local::lib.  A workaround for this is using
C<use lib "$local_lib/lib/perl5";> instead of using C<CPAN::local::lib> directly.

=item * Conflicts with L<ExtUtils::MakeMaker>'s C<PREFIX> option.
C<CPAN::local::lib> uses the C<INSTALL_BASE> option, as it has more predictable and
...
If you've configured CPAN::local::lib to install CPAN modules somewhere in to your
...
CPAN::local::lib looks at the user's C<SHELL> environment variable when printing out
...
Doc patches for a custom CPAN::local::lib directory, more cleanups in the english
...
submitted yet another fix to ensure that CPAN::local::lib can install and bootstrap
...
Improvements to stacking multiple CPAN::local::lib dirs and removing them from the
...
Copyright (c) 2007 - 2013 the CPAN::local::lib L</AUTHOR> and L</CONTRIBUTORS> as
listed above.
